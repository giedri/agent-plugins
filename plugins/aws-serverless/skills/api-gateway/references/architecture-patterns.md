...
- Route different paths (`/service1`, `/service2`, `/docs`) to different backends (ECS, EC2, S3)
- Use `method.request.header.Host` to forward the customer subdomain as a custom header
...
- **Edge-optimized**: Routes requests through CloudFront POPs for optimized TCP connections to global clients. Does **NOT** cache at the edge. For actual edge caching, use a separate CloudFront distribution with a Regional API. Cannot use with private endpoints. ACM certificate must be in `us-east-1`
...
- Prefer Regional with your own CloudFront distribution for maximum control over caching, WAF, and custom behaviors
...
- Set `ApiKeySourceType: AUTHORIZER` so API Gateway reads the key from the authorizer response, not the `x-api-key` header, so tenants never see or manage API keys
- Tiered usage plans (bronze/silver/gold) prevent noisy-neighbor problems
- Onboarding automation (Terraform/SAM): create tenant in IdP + API key in API Gateway + mapping in DynamoDB + associate key with usage plan tier
- Forward `Host` header as custom header to backend for tenant-specific logic
...
**Lambda integrations** (`AWS_PROXY` / `AWS`), the most common integration type. `AWS_PROXY` (Lambda proxy) is the recommended default: API Gateway passes the full request to Lambda and returns the Lambda response directly, no mapping templates needed. `AWS` (Lambda custom) allows VTL request/response transformation but requires more setup. Lambda custom integration is a special case of the AWS service integration type below.
...
- **EventBridge**: event ingestion with routing rules, batching via `#foreach` (REST/WebSocket) or parameter mapping (HTTP)
- **SQS**: async message buffering with query or JSON protocol (REST/WebSocket) or first-class integration (HTTP)
...
- **Kinesis Data Streams**: high-throughput ordered data ingestion with batching (REST/WebSocket) or first-class integration (HTTP)
- **Step Functions**: workflow orchestration (sync Express or async Standard). REST API supports synchronous execution (`StartSyncExecution`) within the integration timeout (29s default, up to 300s Regional/Private). WebSocket API supports two modes: (1) synchronous Express via `StartSyncExecution` (constrained by the 29-second WebSocket integration timeout, not the 5-minute Express maximum), and (2) asynchronous Standard via `StartExecution` with callback (the workflow pushes results back to the client via `POST @connections/{connectionId}` when complete). Lambda durable functions as code-first alternative (REST/WebSocket/HTTP)
...
2. NLB with target group using IP addresses (target type: IP, network: "Other private IP address") to register on-prem server IPs
3. VPC Link to the NLB
4. API Gateway with `VPC_LINK` integration type
   - Disable security group evaluation for PrivateLink on the NLB
   - **AWS Outposts**: Workloads running on Outposts (EC2, ECS, ALB) can also serve as integration targets. Outposts extend the VPC into the on-premises environment, so the same VPC Link + NLB pattern applies. Register Outposts instance IPs in the NLB target group

**Connectivity considerations**: This pattern assumes stable, low-latency connectivity to the on-premises location. AWS Direct Connect provides the most reliable path. Site-to-Site VPN connections are inherently less stable; tunnel flaps cause NLB targets to become unreachable. With default NLB health check settings (30s interval, 3 failures), there is a ~90-second window where API Gateway sends traffic to unreachable targets, resulting in integration timeouts. Tune NLB health check intervals (10s, 2 failures) and set API Gateway integration timeouts to match your SLA. Implement a `/health` endpoint on the on-premises target that validates downstream dependencies. Monitor NLB `UnHealthyHostCount` and alarm on it
...
- **Pattern 2**: PrivateLink between accounts with VPC endpoint service
- **Pattern 3**: Transit Gateway connecting VPCs across accounts
- Use `x-apigw-api-id` header to route to correct API when multiple private APIs share a VPC endpoint
...
- **Regional API + restrictions** (defense-in-depth, not a security boundary): Keep a regional endpoint but restrict direct access. Use a custom header from CloudFront (via origin custom headers) and validate it in a Lambda authorizer. Combine with disabling the default `execute-api` endpoint to force traffic through the custom domain fronted by CloudFront. **Caveat**: The header value is a static secret that cannot be cryptographically verified. If leaked through logs, source code, or developer machines, attackers can bypass CloudFront. Rotate the value regularly, store it in AWS Secrets Manager, and treat it as a credential. For high-security workloads, use the private API approach above instead
- **WAF on API Gateway**: Attach WAF directly to the REST API as an additional layer, so even if CloudFront is bypassed, WAF rules still apply

### On-Premises Access to Private APIs
- AWS Direct Connect or Site-to-Site VPN to reach VPC
- Route 53 Resolver inbound endpoints for on-premises DNS resolution of VPC endpoint DNS names

## VPC Links

VPC Links enable API Gateway to reach **private integration targets** inside a VPC (backends running on ALB, NLB, ECS, EC2, or Cloud Map services that are not publicly accessible). API Gateway creates a private connection to the VPC without exposing the backend to the internet.
...
- These are separate resource types and cannot be interchanged
- **Not the same as private endpoints**: A *private API endpoint* restricts who can **call** the API (only from within a VPC via `execute-api` VPC endpoint). A *VPC Link* controls where the API **forwards requests to** (private backends in a VPC). These are independent: a public API can use VPC Links to reach private backends, and a private API can call public HTTP endpoints without VPC Links
...
- **Data sovereignty**: Latency-based routing may route EU users to US regions (or vice versa) if latency is lower, potentially violating GDPR or other data residency requirements. Use geo-based routing when data sovereignty is a concern. Note that DynamoDB Global Tables replicate data to all configured regions regardless of routing, so do not add regions that would violate data residency constraints
- Requires data replication strategy: DynamoDB Global Tables for state, S3 Cross-Region Replication for objects, or application-level sync
- **Conflict resolution**: DynamoDB Global Tables uses last-writer-wins: concurrent writes to the same item in different regions within the replication lag window result in one write being silently discarded. **Conditional writes do NOT prevent cross-region conflicts**; they only enforce conditions locally before replication. For conflict-sensitive data (financial balances, counters, inventory), use single-region write routing (active-passive for writes, active-active for reads) or design all writes as commutative operations (e.g., `ADD` for counters, append-only logs) so replication order does not matter
...
- **Request validation at the front door**: Use API Gateway validators (headers, query strings, JSON schema) to reject bad requests before they reach the backend ("easiest task is the one you don't have to do")
